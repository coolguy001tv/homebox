...
- **威联通 NAS 缩略图匹配规则修正**：`findNasThumb` 函数新增 `default` 前缀匹配逻辑。威联通（QNAP）NAS 在 `.@__thumb/` 目录下的缩略图实际命名为 `default` + 原始文件名（如 `defaultIMG_1234.HEIC`），而非完全同名。修改后优先尝试精确同名匹配（兼容其他 NAS），若未命中则尝试 `default` 前缀匹配，确保威联通 NAS 的缩略图能被正确读取。
